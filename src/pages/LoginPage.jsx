...
import { Mail, Lock, User, Phone, Eye, EyeOff, ArrowLeft, CheckCircle, AlertCircle } from 'lucide-react';
...
const LoginPage = () => {
  ...
  const validateForm = () => {
    ...
    if (loginMethod !== 'phone' && !formData.password) {
    ...
    try {
      if (loginMethod === 'phone') {
        ...
        if (result.success) {
          ...
          setErrors({ phone: result.error || result.message });
        }
      } else {
        const credentials = {};
        ...
        if (result.success) {
          ...
          setErrors({ submit: result.error || result.message });
        }
      }
    } catch (error) {
  ...
  const handleOTPVerify = async (otp) => {
    try {
      const result = await login({ phone: formData.phone, otp }, 'otp');
      if (result.success) {
        setSuccess('Login successful! Redirecting...');
        setTimeout(() => navigate('/'), 1500);
        return true;
